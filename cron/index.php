...
class DatabaseBackup
{
    ...
    $this->database = $database;
	  ...
	  // $image_query = mysqli_query($result,"SELECT * FROM `user` WHERE `role`=1 and `email_status`='verified' and `business_status`=1 and `company_db_name`!=''");
	   $image_query = mysqli_query($result,"SELECT * FROM `user` WHERE `role`=1 and `email_status`='verified' and `business_status`=1 and `company_db_name`!=''");
	   $image_query = mysqli_query($result,"SELECT user.* FROM `user` LEFT JOIN company_settings on company_settings.c_id = user.c_id WHERE user.role =1 and user.email_status ='verified' and user.business_status =1 and user.company_db_name!='' and (company_settings.backup_frequency='weekly' or company_settings.backup_frequency is NULL)");
		$resultArray = array();
